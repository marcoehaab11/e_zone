...
public sealed class PublicController(AppDbContext db) : ControllerBase {
 ...
 [HttpGet("advertisements")] public async Task<IActionResult> Ads() {var now=DateTime.UtcNow;var x=await db.Advertisements.AsNoTracking().Where(a=>a.IsActive&&!a.IsDeleted&&(!a.StartDate.HasValue||a.StartDate<=now)&&(!a.EndDate.HasValue||a.EndDate>=now)).OrderBy(a=>a.DisplayOrder).ToListAsync();return Ok(new ApiResponse<object>(true,"طھظ… ط§ظ„ط§ط³طھط±ط¬ط§ط¹",x));}
