...
public sealed class PublicController(AppDbContext db) : ControllerBase {
 ...
 [HttpGet("suppliers/{id:int}")] public async Task<IActionResult> Supplier(int id) { var item=await db.Suppliers.AsNoTracking().Where(x=>x.Id==id&&x.IsActive&&!x.IsDeleted).Select(x=>new SupplierDetailsDto(x.Id,x.Name,x.Description,x.LogoUrl,x.PhoneNumber,x.AdditionalPhoneNumbers,x.WhatsAppNumber,x.Email,x.Website,x.Address,x.ShortAddress,x.HasTechnicians,x.Latitude,x.Longitude,x.GoogleMapsUrl,x.Images.OrderBy(i=>i.DisplayOrder).Select(i=>new{i.Id,i.ImageUrl,i.DisplayOrder}),x.SupplierCategories.Select(c=>new{c.CategoryId,c.Category.Name}),x.SupplierAreas.Select(a=>new{a.AreaId,a.Area.Name}))).FirstOrDefaultAsync(); return item is null?NotFound(new ApiResponse<object>(false,"ط§ظ„ظ…ظˆط±ط¯ ط؛ظٹط± ظ…ظˆط¬ظˆط¯",null)):Ok(new ApiResponse<object>(true,"طھظ… ط§ظ„ط§ط³طھط±ط¬ط§ط¹",item)); }
