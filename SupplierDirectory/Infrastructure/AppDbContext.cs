...
public sealed class AppDbContext(DbContextOptions<AppDbContext> options) : IdentityDbContext<ApplicationUser>(options)
{
 ...
 protected override void OnModelCreating(ModelBuilder b) { base.OnModelCreating(b);
  b.Entity<Area>().HasQueryFilter(x=>!x.IsDeleted); b.Entity<Category>().HasQueryFilter(x=>!x.IsDeleted); b.Entity<ProductCategory>().HasQueryFilter(x=>!x.IsDeleted); b.Entity<Product>().HasQueryFilter(x=>!x.IsDeleted); b.Entity<ProductImage>().HasQueryFilter(x=>!x.IsDeleted && !x.Product.IsDeleted); b.Entity<Service>().HasQueryFilter(x=>!x.IsDeleted); b.Entity<ServiceImage>().HasQueryFilter(x=>!x.IsDeleted && !x.Service.IsDeleted); b.Entity<Supplier>().HasQueryFilter(x=>!x.IsDeleted); b.Entity<SupplierImage>().HasQueryFilter(x=>!x.IsDeleted && !x.Supplier.IsDeleted); b.Entity<SupplierCategory>().HasQueryFilter(x=>!x.Supplier.IsDeleted && !x.Category.IsDeleted); b.Entity<SupplierArea>().HasQueryFilter(x=>!x.Supplier.IsDeleted && !x.Area.IsDeleted); b.Entity<Advertisement>().HasQueryFilter(x=>!x.IsDeleted);
